...
{
    class AzureAuthHelper : IAuthHelper
    {
        ...
        {
          CommonHelper.WriteLog("SignInAsync ENTRY", 3);
            ...
            CommonHelper.WriteLog("SignInAsync EXIT", 3);
            return newContext;
        }

        public async Task<UserContext> AcquireTokenAsync()
        {
            CommonHelper.WriteLog("AqqureTokenAsync ENTRY", 3);
            UserContext newContext = null;
            try
            {
                // acquire token silent
            ...
            CommonHelper.WriteLog("AqqureTokenAsync EXIT", 3);
            return newContext;
        }

        public async Task<UserContext> SignInInteractivelyAsync()
        {
            CommonHelper.WriteLog("SignInInteractivelyAsync ENTRY", 3);
            UserContext newContext = null;
            try
            {
                // acquire token silent
            ...
            CommonHelper.WriteLog("SignInInteractivelyAsync EXIT", 3);
            return newContext;
        }

        private async Task<UserContext> AcquireToken()
        {
            //CommonHelper.WriteLog("AcquireToken (Silent) ENTRY", 3);
        ...
        {
            IEnumerable<IAccount> accounts = await PCA.GetAccountsAsync();
           // CommonHelper.WriteLog("SignInInteractively ENTRY", 3);
            //CommonHelper.WriteLog(ParentActivityOrWindow.ToString(), 3);
            AuthenticationResult authResult = await PCA.AcquireTokenInteractive(B2CConstants.Scopes)
            ...
            var newContext = UpdateUserInfo(authResult);
           // CommonHelper.WriteLog("SignInInteractively EXIT", 3);
            return newContext;
        }
        ...
        {
          //  CommonHelper.WriteLog("Setting parent", 3);
